...
class SubscribersCreator:
  ...
  def create(self,http_request:HttpRequest) -> HttpResponse:
    ...
    self.__check_sub(subscriber_email,evento_id) #Verifica se houve um inscrito com o mesmo nome
    self.__insert_sub(subscriber_info) #Insere um novo inscrito
  ...
  def  __check_sub(self,subscriber_email:str,evento_id:int) -> None:
    ...
    if response: raise Exception("Subscribe Already exist !") 

  def __insert_sub(self,subscriber_info:dict) -> None:
    self.__subs_repo.insert(subscriber_info)

  def __format_response(self,subscriber_info:dict) -> HttpResponse: #Traduz a resposta do SubscribersCreator para um HttpResponse
